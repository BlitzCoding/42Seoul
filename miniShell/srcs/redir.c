...
/*   Updated: 2022/08/01 00:47:13 by seuhan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	*f = open(lst_cmd->lst_out->file, O_TRUNC | O_RDWR | O_CREAT, 0755);
	if (*f < 0)
	{
		close(*f);
...
int	open_redir2(t_lst_cmd *lst_cmd, struct stat *s)
{
	...
	while (lst_cmd->lst_out->next)
	{
		...
		{
			f = open(lst_cmd->lst_out->file, O_APPEND | O_RDWR | O_CREAT, 0755);
			if (f < 0)
			{
				close(f);
				if (!S_ISDIR(s->st_mode) && !S_ISREG(s->st_mode))
